...
async function seedPins(options: SeedOptions = {}) {
  ...
  try {
    ...
    if (existingUsers.length === 0) {
      ...
      console.log(
        `   2. Then run: pnpm --filter @pinsquirrel/database seed:pins`
      )
      console.log(
        ...
        columns: {
          ...
          description: funcs.valuesFromArray({
            values: Array.from(
              { length: 30 },
              (_, i) =>
                `This is a description for article ${i + 1}. It provides useful information about the topic.`
            ),
          }),
    ...
    const uniqueRelationships = relationships.filter(
      (rel, index, self) =>
        index ===
        self.findIndex(r => r.pinId === rel.pinId && r.tagId === rel.tagId)
    )
    ...
    const stats = await Promise.all(
      existingUsers.map(async user => {
        const totalPins = await db
          .select()
          .from(pins)
          .where(eq(pins.userId, user.id))
        const readLaterPins = await db
          .select()
          .from(pins)
          .where(and(eq(pins.userId, user.id), eq(pins.readLater, true)))
    ...
    stats.forEach(stat => {
      console.log(
        `   👤 ${stat.username}: ${stat.total} pins (${stat.readLater} read later)`
      )
    })
...
console.log(
  `   --seed=N    Seed value for deterministic generation (default: 42)`
)
console.log(``)
