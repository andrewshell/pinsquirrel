 ...
 *
 * All it does is hand `initBackground` the things it cannot do itself - a real
 * sync over the stored connection, the two halves of a real OAuth flow, and
 * somewhere to put a failure nobody is waiting for - so that the wiring in
 ...
 */
import { cancelConnect, completeConnect, startConnect } from './auth.ts'
