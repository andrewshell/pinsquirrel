...
- **Note (when done):** Root `eslint.config.base.js` exports `createConfig(tsconfigRootDir, { strict })`; the four plugin packages moved to the root `package.json` so only that file imports them, and every package config is now four lines plus its `ignores`. `libs/adapters` passes `strict: true` and still passes `strictTypeChecked`. There are no accessibility rules and never were — no a11y plugin is installed anywhere — so CLAUDE.md now says what is true instead.

  The stricter lint surfaced **345 findings**, none of them a bug. 323 were `unbound-method` on `expect(mock.method)` in service tests, which is exactly what a mock assertion does and has no vitest-aware variant of the rule — turned off for test files in the base, alongside the `unsafe-*` rules already off there. 5 more were `require-await` on `libs/crypto`'s exported entry points, whose promises are the contract every caller awaits even though `node:crypto`'s sync primitives do the work — disabled per file with the reason. The remaining 17 were fixed: `null as any` casts and their now-unused disable directives in `url.test.ts`, an `any` returned from a `.catch` in `metadata.test.ts`, three redundant `as unknown as` mock casts, seven `async` test hooks/helpers with nothing to await, and two `result.map(this.mapToTag)` calls in `DrizzleTagRepository` — the one finding in production code. That last one works today only because `mapToTag` never reads `this`; it is now `map(tag => this.mapToTag(tag))`.
...
- **Note (when done, DNS half):** The range list moved to `libs/domain/src/ip-address.ts` so both halves decide from one copy; it is pure, with no imports at all — the one thing `net.isIP` was used for there is a dotted-quad test, spelled out. `libs/adapters` gained `undici` as a direct dependency and `NodeHttpFetcher` now fetches through an `Agent` whose `connect.lookup` resolves the hostname and refuses if **any** answer is private, surfacing as the existing `InvalidUrlError`. There is no TOCTOU window left: the check runs as the connection is made rather than before the fetch, and every redirect hop connects through the same dispatcher, so a redirect into a private range is refused too. The global `fetch` could not be used — Node's built-in fetch rejects a dispatcher from a separately installed undici ("invalid onRequestStart method"), verified.
